...
import { field, label, input, select, primaryBtn, actionBtn, ghostBtn } from './formStyles'
...
export default function CashRegister() {
  ...
  const byType = useMemo(() => {
    ...
    for (const t of transactions) {
      const name = t.custom_type?.type_name || t.type || 'Other'
      m[name] = (m[name] || 0) + Number(t.amount)
    }
    return m
  }, [transactions])
  ...
  async function saveEdit(id) {
    ...
    if (!e) { setEditId(null); setOk('Updated.'); refresh() }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 10, marginBottom: 18 }}>
        <StatCard label="Cash in Hand" value={rs(Math.abs(summary.cashInHand))}
          accent={summary.cashInHand >= 0 ? 'success' : 'danger'}
          sub={summary.cashInHand < 0 ? '⚠ Deficit' : 'Available'} />
        <StatCard label="Manager Balance" value={rs(Math.abs(summary.managerBalance))}
          accent={summary.managerBalance > 0 ? 'warning' : summary.managerBalance < 0 ? 'state-blue' : 'success'}
          sub={summary.managerBalance > 0 ? 'You owe manager' : summary.managerBalance < 0 ? 'Manager owes you' : 'Settled'} />
        {Object.entries(byType).slice(0, 4).map(([name, total]) => (
          <StatCard key={name} label={name.length > 16 ? name.slice(0, 16) + '…' : name} value={rs(total)} accent="gold" />
        ))}
      </div>
      ...
      {isClerk && (
        <div style={{ background: 'var(--bg-surface)', border: '1px dashed var(--border)', borderRadius: 'var(--radius-md)', padding: 16, marginBottom: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--gold)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Log Transaction</div>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <div style={{ ...field, marginBottom: 0, minWidth: 120 }}><label style={label}>Date</label><input style={input} type="date" value={date} onChange={e => setDate(e.target.value)} /></div>
            <div style={{ ...field, marginBottom: 0, flex: 2, minWidth: 150 }}><label style={label}>Type *</label>
            ...
            <div style={{ ...field, marginBottom: 0, flex: 2, minWidth: 140 }}><label style={label}>Description</label><input style={input} value={desc} onChange={e => setDesc(e.target.value)} placeholder="Details…" /></div>
            <div style={{ ...field, marginBottom: 0, minWidth: 100 }}><label style={label}>Amount *</label><input style={input} type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Rs." /></div>
            <div style={{ ...field, marginBottom: 0, minWidth: 110 }}><label style={label}>Remarks</label><input style={input} value={remarks} onChange={e => setRemarks(e.target.value)} /></div>
            <button style={{ ...primaryBtn, width: 'auto', padding: '11px 20px', alignSelf: 'flex-end' }} onClick={addTransaction} disabled={saving}>{saving ? '…' : 'Add'}</button>
          </div>
          {selectedType && (
            <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text-muted)' }}>
              Effect: <span style={{ color: isIn ? 'var(--success)' : 'var(--danger)', fontWeight: 700 }}>{isIn ? '↑ Cash in hand increases' : '↓ Cash in hand decreases'}</span>
              {selectedType.manager_balance_effect !== 'none' && <span style={{ marginLeft: 8 }}>· Manager balance: {selectedType.manager_balance_effect === 'increase' ? '↑' : '↓'}</span>}
            </div>
          )}
        </div>
      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>
                ...
                <th style={{ ...th, textAlign: 'right' }}>Amount</th><th style={th}>Remarks</th>
              ...
              {transactions.map(t => {
                ...
                return editId === t.id ? (
                  ...
                  <tr key={t.id}>
                    ...
                    <td style={td}><span style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--text-secondary)', background: 'var(--bg-elevated)', padding: '2px 8px', borderRadius: 999 }}>{ct?.type_name || t.type}</span></td>
                    ...
                    <td style={{ ...td, color: 'var(--text-muted)' }}>{t.remarks || '—'}</td>
                    {isClerk && (
                      <td style={td}>
                        <button style={{ ...actionBtn, marginRight: 4 }} onClick={() => { setEditId(t.id); setEditDraft({ entry_date: t.entry_date, description: t.description, amount: t.amount, remarks: t.remarks }) }}>Edit</button>
                        <button style={actionBtn} onClick={() => archiveTxn(t.id)}>Archive</button>
                      </td>
                    )}
                  </tr>
                )
